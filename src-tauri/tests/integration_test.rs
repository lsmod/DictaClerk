...
async fn test_encoder_integration() {
    ...
    println!("Compression ratio: {:.2}x", wav_size as f64 / ogg_size as f64);
    
    // OGG should be significantly smaller than WAV
    assert!(ogg_size < wav_size / 2, "OGG file is not significantly smaller than WAV");
}
...
async fn test_encoder_with_different_durations() {
    ...
    for duration in durations {
        ...
        let result = encoder
            .encode(&wav_path, Some(&ogg_path), None)
            .await
            .expect(&format!("Encoding failed for duration {}", duration));
        
        // Verify
        assert!(ogg_path.exists(), "OGG file was not created for duration {}", duration);
        assert!(result.actual_size.unwrap() > 0, "OGG file is empty for duration {}", duration);
        
        println!("Duration: {}s, OGG size: {} bytes", duration, result.actual_size.unwrap());
    }
} 